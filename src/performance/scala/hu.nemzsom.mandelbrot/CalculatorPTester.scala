...
case class CalcPTest(width: Int, height: Int, topLeft: Complex, mathHeight: BigDecimal, maxIter: Int, times: Int) extends PerformanceTestSpec
...
object CalculatorPTester extends App {
  ...
  val mc = new MathContext(50)

  val testCases: List[(CalcPTest, Range)] = List(
    CalcPTest(640, 480, Complex(BigDecimal(-2, mc), BigDecimal(-1.65, mc)), BigDecimal(2.5,mc), 1000, 100) -> (1 to 8)/*,
